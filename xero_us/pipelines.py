...
class XeroUsPipeline(object):
    ...
    def __init__(self):
        ...
        self.seenD = set() #Item Email Domain Name seen in previou scrapes
    ...
    def process_item(self, item, spider):
        ...
        compName = item['companyName'].lower()
        ...
        if tempName in self.seenN:
            if compName in self.seenC:
                raise DropItem('Duplicate NAME AND EMAIL found %s, %s', tempName, compName)
                drop = True
            try:
                if email.split("@")[1] in self.seenD:
                    raise DropItem('Duplicate NAME AND EMAIL found %s, %s', tempName, compName)
                    drop = True
            except IndexError:
                pass
        ...
        if not drop:
            ...
            listItem.fieldValues({"61":"8","58":contact.email(),"51":string.capwords(item['companyName']),"69":level,"72":"0","87":"1","100":"2","55":item['phoneNumber']})
